  ...
  return (
    <div className="pdf-viewer">
      <div className="pdf-toolbar">
        ...
        <button type="button" onClick={() => setScale((value) => Math.max(0.5, value - 0.25))}>−</button>
    ...
    <div className="file-viewer-overlay" role="presentation" onClick={onClose}>
      <section className="file-viewer" role="dialog" aria-modal="true" aria-label={`Preview ${preview?.name || fallbackName}`} onClick={(event) => event.stopPropagation()}>
        <header className="file-viewer-header">
          <div><strong>{preview?.name || fallbackName}</strong>{preview && <span>{preview.mime_type || "File"} · {formatBytes(preview.size)}</span>}</div>
